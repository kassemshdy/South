...
def category(db: Session) -> Category:
    entity = Category(name_ar="مطاعم ومأكولات", slug=f"restaurants-{uuid.uuid4().hex[:6]}", sort_order=1)
    db.add(entity)
    db.commit()
    return entity
...
def location(db: Session) -> Location:
    entity = Location(
        name_ar="صور", slug=f"tyre-{uuid.uuid4().hex[:6]}", type=LocationType.DISTRICT
    )
    db.add(entity)
    db.commit()
    return entity
